...
final class NativeCliApplication
{
    ...
    public function run(array $argv): int
    {
        ...
        try {
            return match ($command) {
                ...
                'ai:providers' => $this->aiProviders($opts),
                'ai:analyze-site' => $this->aiAnalyzeSite($args, $opts),
                'ai:explain-plan' => $this->aiExplainPlan($args, $opts),
                'search:providers' => $this->searchProviders($opts),
                ...
                'ml:strategies' => $this->mlStrategies($args, $opts),
                'ml:train' => $this->mlTrain($args, $opts),
                'ml:score' => $this->mlScore($args, $opts),
                'ml:adaptive-plan' => $this->mlAdaptivePlan($args, $opts),
                'ml:feedback' => $this->mlFeedback($args, $opts),
                'ml:export-training' => $this->mlExportTraining($args, $opts),
                ...
                'extract:recipe' => $this->extractionRecipe($args, $opts),
                'extract:quality' => $this->extractionQuality($args, $opts),
                'extract:recipes' => $this->extractionRecipes($opts),
                ...
                'publisher:list' => $this->publisherList($args, $opts),
                'publisher:show' => $this->publisherShow($args, $opts),
                'publisher:seeds' => $this->publisherSeeds($args, $opts),
                'publisher:plan' => $this->publisherPlan($args, $opts),
                'publisher:graph' => $this->publisherGraph($args, $opts),
                'publisher:enterprise-plan' => $this->publisherEnterprisePlan($args, $opts),
                'publisher:extract-article' => $this->publisherExtractArticle($args, $opts),
                'publisher:schema' => $this->publisherSchema($args, $opts),
                'publisher:normalize' => $this->publisherNormalize($args, $opts),
    ...
    {
        $this->out('MNB ScraperKit 1.0.3 - Professional Symfony Console CLI');
    ...
    private function listCommands(): int
    {
        $commands = [
            ...
            'ai:providers' => 'List optional AI crawl-analysis providers and configuration status.',
            'ai:analyze-site <url|html-file>' => 'Analyze crawl flexibility, extraction opportunities, risks, page types, recipes, and safe crawl limits.',
            'ai:explain-plan <analysis.json>' => 'Explain an AI/rule-based crawl flexibility report in concise operator-friendly text.',
            'search:providers' => 'List safe search discovery providers and API/import configuration status.',
            ...
            'publisher:list' => 'List safe academic publisher metadata crawl targets.',
            'publisher:show <publisher>' => 'Show one publisher crawl profile, safe modes, and seed URLs.',
            'publisher:seeds' => 'Export publisher seed URLs as JSON, CSV, or TXT.',
            'publisher:plan' => 'Generate safe metadata-only crawl job plans for publisher seeds.',
            'publisher:graph <publisher>' => 'Show publisher navigation graph from indexes to articles/books/chapters.',
            'publisher:enterprise-plan <publisher>' => 'Generate enterprise crawl workflow steps for journals, books, issues, and article metadata.',
            ...
            'extract:recipe <html-file>' => 'Run an explainable extraction recipe with field provenance and quality report.',
            'extract:quality <result.json>' => 'Score extraction output for completeness, provenance, component counts, and missing required fields.',
            'extract:recipes' => 'List bundled extraction recipe files for explainable extraction workflows.',
    ...
    private function templateList(array $args, array $opts): int
    {
        ...
        if ($this->bool($opts, 'json')) {
            $this->outJson(['ok' => true, 'template_version' => '1.0.3', 'templates' => $templates]);
            return 0;
        }
    ...
    private function presetList(array $args, array $opts): int
    {
        ...
        if ($this->bool($opts, 'json')) {
            $this->outJson(['ok' => true, 'preset_pack_version' => '1.0.3', 'preset_packs' => $packs]);
            return 0;
        }
    ...
    private function annotationExport(array $args, array $opts): int
    {
        ...
        $this->exportRows($rows, $output, $format, ['annotation_export_version' => '1.0.3', 'rows_total' => count($rows)]);
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlStrategies(array $args, array $opts): int
    {
        $path = $this->rootDir . '/config/ml/crawl-strategies.json';
        $data = is_file($path) ? json_decode((string) file_get_contents($path), true) : [];
        if (!is_array($data)) {
            $data = ['ml_crawl_strategy_version' => '1.0.3', 'strategies' => []];
        }
        $this->outJson($data);
        return 0;
    }
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlTrain(array $args, array $opts): int
    {
        $positiveFile = $this->optString($opts, 'positive') ?? $this->optString($opts, 'positives');
        $negativeFile = $this->optString($opts, 'negative') ?? $this->optString($opts, 'negatives');
        $feedbackFile = $this->optString($opts, 'feedback');
        if (!$positiveFile && !$feedbackFile) {
            throw new \InvalidArgumentException('Usage: ml:train --positive=positive-urls.txt --negative=negative-urls.txt [--output=storage/ml/model.json]');
        }
        $positive = $positiveFile && is_file($positiveFile) ? $this->readUrlsForIntelligence($positiveFile) : [];
        $negative = $negativeFile && is_file((string) $negativeFile) ? $this->readUrlsForIntelligence((string) $negativeFile) : [];
        if ($feedbackFile) {
            $feedback = (new CrawlFeedbackStore($this->rootDir))->positivesNegatives((string) $feedbackFile);
            $positive = array_merge($positive, $feedback['positive'] ?? []);
            $negative = array_merge($negative, $feedback['negative'] ?? []);
        }
        if ($positive === [] || $negative === []) {
            throw new \InvalidArgumentException('ML training needs at least one positive and one negative URL example.');
        }
        $model = (new CrawlMlModel())->train($positive, $negative, [
            'min_token_length' => (int) ($this->optString($opts, 'min-length', '3') ?? '3'),
            'score_threshold' => (float) ($this->optString($opts, 'threshold', '0.58') ?? '0.58'),
        ]);
        $output = $this->optString($opts, 'output') ?: $this->rootDir . '/storage/ml/crawl-model.json';
        $this->writeJson($output, $model);
        $this->outJson(['ok' => true, 'output' => $output, 'training_summary' => $model['training_summary'] ?? []]);
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlScore(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        $modelPath = $this->optString($opts, 'model') ?? $this->optString($opts, 'model-file');
        if (!$input || !is_file((string) $input) || !$modelPath || !is_file((string) $modelPath)) {
            throw new \InvalidArgumentException('Usage: ml:score <urls.txt|results.json> --model=model.json [--output=scores.json]');
        }
        $model = json_decode((string) file_get_contents((string) $modelPath), true);
        if (!is_array($model)) {
            throw new \RuntimeException('Model file must be JSON: ' . $modelPath);
        }
        $data = (new CrawlMlModel())->scoreUrls($model, $this->readUrlsForIntelligence((string) $input));
        $format = strtolower($this->optString($opts, 'format', 'json') ?? 'json');
        $output = $this->optString($opts, 'output');
        if ($output) {
            if ($format === 'txt') {
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlAdaptivePlan(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        if (!$input || !is_file((string) $input)) {
            throw new \InvalidArgumentException('Usage: ml:adaptive-plan <urls.txt|results.json> [--model=model.json] [--crawl-budget=25] [--output=plan.json]');
        }
        $modelPath = $this->optString($opts, 'model') ?? $this->optString($opts, 'model-file');
        $model = null;
        if ($modelPath) {
            $modelData = json_decode((string) file_get_contents((string) $modelPath), true);
            if (!is_array($modelData)) {
                throw new \RuntimeException('Model file must be JSON: ' . $modelPath);
            }
            $model = $modelData;
        }
        $data = (new AdaptiveCrawlPlanner())->plan($this->readUrlsForIntelligence((string) $input), $model, [
            'crawl_budget' => (int) ($this->optString($opts, 'crawl-budget', $this->optString($opts, 'max-pages', '25')) ?? '25'),
            'explore_ratio' => (float) ($this->optString($opts, 'explore-ratio', '0.15') ?? '0.15'),
            'score_threshold' => (float) ($this->optString($opts, 'threshold', '0.55') ?? '0.55'),
        ...
        if ($output) {
            if ($format === 'txt') {
                $this->writeTextLines($output, array_values(array_map('strval', $data['selected_urls'] ?? [])));
            } else {
                $this->writeJson($output, $data);
            }
            $this->outJson(['ok' => true, 'output' => $output, 'selected_total' => $data['selected_total'] ?? 0]);
        } else {
            $this->outJson($data);
        }
        return 0;
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlFeedback(array $args, array $opts): int
    {
        $url = $args[0] ?? $this->optString($opts, 'url');
        if (!$url) {
            throw new \InvalidArgumentException('Usage: ml:feedback <url> --label=relevant|irrelevant|review [--feedback=storage/ml/crawl-feedback.jsonl]');
        }
        $result = (new CrawlFeedbackStore($this->rootDir))->add([
            'url' => (string) $url,
            'label' => $this->optString($opts, 'label', 'relevant') ?? 'relevant',
            'reason' => $this->optString($opts, 'reason') ?? $this->optString($opts, 'note') ?? '',
            'source' => $this->optString($opts, 'source', 'manual') ?? 'manual',
            'profile' => $this->optString($opts, 'profile', '') ?? '',
        ], $this->optString($opts, 'feedback'));
        $this->outJson($result);
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function mlExportTraining(array $args, array $opts): int
    {
        $positiveFile = $this->optString($opts, 'positive') ?? $this->optString($opts, 'positives');
        $negativeFile = $this->optString($opts, 'negative') ?? $this->optString($opts, 'negatives');
        $feedbackFile = $this->optString($opts, 'feedback');
        ...
        if ($positive === [] && $negative === []) {
            throw new \InvalidArgumentException('Usage: ml:export-training --positive=positive.txt --negative=negative.txt [--format=jsonl]');
        }
        $exporter = new TrainingDataExporter();
        $data = $exporter->build($positive, $negative);
        $format = strtolower($this->optString($opts, 'format', 'jsonl') ?? 'jsonl');
        $output = $this->optString($opts, 'output') ?: $this->rootDir . '/storage/ml/training-data.' . ($format === 'json' ? 'json' : 'jsonl');
        if ($format === 'json') {
            $this->writeJson($output, $data);
        } else {
            $exporter->writeJsonl($data, $output);
        }
        $this->outJson(['ok' => true, 'output' => $output, 'records_total' => $data['records_total'] ?? 0, 'format' => $format]);
        return 0;
    }
    ...
    /** @param array<string,mixed> $opts */
    private function aiProviders(array $opts): int
    {
        $data = AiProviderRegistry::summary();
        if ($this->bool($opts, 'json')) {
            $this->outJson($data);
            return 0;
        }
        $this->out('AI crawl analysis providers:');
        foreach ((array) $data['providers'] as $provider) {
            if (!is_array($provider)) {
                continue;
            }
            $this->out(sprintf('  - %-16s configured=%s network=%s', (string) ($provider['id'] ?? ''), !empty($provider['configured']) ? 'yes' : 'no', !empty($provider['requires_network']) ? 'yes' : 'no'));
        }
        $this->out('Default: rule_based. External AI is disabled by default; wire approved adapters before sending page content outside your environment.');
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function aiAnalyzeSite(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input') ?? $this->optString($opts, 'url');
        if (!$input) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper ai:analyze-site <url|html-file> [--url=https://example.com/page] [--goal=article_metadata] [--provider=rule_based] [--output=file]');
        }
        $goal = $this->optString($opts, 'goal', 'general_metadata') ?? 'general_metadata';
        $provider = $this->optString($opts, 'provider', 'rule_based') ?? 'rule_based';
        $html = null;
        $url = (string) $input;
        if (is_file((string) $input)) {
            $html = (string) file_get_contents((string) $input);
            $url = $this->optString($opts, 'url') ?: $this->optString($opts, 'base-url') ?: 'https://example.com/';
        }
        $data = (new SiteCrawlAnalyzer($this->rootDir))->analyze($url, $html, [
            'goal' => $goal,
            'provider' => $provider,
        ]);
        $output = $this->optString($opts, 'output');
        ...
        if ($this->bool($opts, 'json') || !$output) {
            ...
            $this->out('AI crawl analysis output: ' . $output);
            $this->out('Flexibility: ' . (string) ($data['crawl_flexibility'] ?? '') . ' score=' . (string) ($data['flexibility_score'] ?? '') . ' risk=' . (string) ($data['risk'] ?? ''));
        }
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function aiExplainPlan(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        if (!$input || !is_file((string) $input)) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper ai:explain-plan <analysis.json> [--json]');
        }
        $data = json_decode((string) file_get_contents((string) $input), true);
        if (!is_array($data)) {
            throw new \RuntimeException('Invalid analysis JSON: ' . $input);
        }
        $explanation = [
            'ok' => true,
            'ai_explanation_version' => '1.0.3',
            'url' => (string) ($data['url'] ?? ''),
            'summary' => sprintf(
                'Flexibility is %s (%s/100), risk is %s, recommended mode is %s.',
                (string) ($data['crawl_flexibility'] ?? 'unknown'),
                (string) ($data['flexibility_score'] ?? '0'),
                (string) ($data['risk'] ?? 'unknown'),
                (string) ($data['recommended_mode'] ?? 'unknown')
            ),
            'recommended_next_steps' => array_values(array_filter([
                'Start with sitemap/RSS/API/DOI discovery before crawling HTML pages.',
                ((string) ($data['risk'] ?? '') === 'high') ? 'Use metadata-only extraction, low rates, robots checks, and authorization review.' : null,
                ((bool) ($data['requires_browser'] ?? false)) ? 'Use browser-assisted crawling only for public or authorized pages.' : null,
                'Run suggested extraction recipe(s): ' . implode(', ', (array) ($data['suggested_recipes'] ?? [])),
            ])),
            'safe_limits' => $data['safe_limits'] ?? [],
            'detected_page_types' => $data['detected_page_types'] ?? [],
            'extractable_fields' => $data['extractable_fields'] ?? [],
        ];
        if ($this->bool($opts, 'json')) {
            $this->outJson($explanation);
        } else {
            $this->out($explanation['summary']);
            foreach ((array) $explanation['recommended_next_steps'] as $step) {
                $this->out('  - ' . (string) $step);
            }
        }
        return 0;
    }
    ...
    /** @param array<string,mixed> $opts */
    private function searchProviders(array $opts): int
    {
        $data = SearchProviderRegistry::summary();
        if ($this->bool($opts, 'json')) {
            $this->outJson($data);
            return 0;
        }
        $this->out('Search discovery providers:');
        foreach ((array) $data['providers'] as $provider) {
            if (!is_array($provider)) {
                continue;
            }
            $this->out(sprintf('  - %-22s configured=%s network=%s', (string) ($provider['id'] ?? ''), !empty($provider['configured']) ? 'yes' : 'no', !empty($provider['requires_network']) ? 'yes' : 'no'));
        }
        $this->out('Search result page scraping is intentionally unsupported. Use approved APIs or offline/imported result JSON.');
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function searchWeb(array $args, array $opts): int
    {
        $query = $args[0] ?? $this->optString($opts, 'query');
        if (!$query) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper search:web "query" [--provider=google_custom_search] [--input=results.json] [--limit=20] [--output=file]');
        }
        $data = (new SearchDiscoveryEngine())->search((string) $query, [
            'provider' => $this->optString($opts, 'provider', 'offline'),
            'input' => $this->optString($opts, 'input'),
            'limit' => (int) $this->opt($opts, 'limit', 10),
            'goal' => $this->optString($opts, 'goal', ''),
        ]);
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $data);
        ...
        if ($this->bool($opts, 'json') || !$output) {
            ...
            $this->out('Search discovery output: ' . $output);
            $this->out('Results: ' . (string) ($data['results_total'] ?? 0) . ' status=' . (string) ($data['status'] ?? ''));
        }
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function searchDiscover(array $args, array $opts): int
    {
        $query = $args[0] ?? $this->optString($opts, 'query');
        if (!$query) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper search:discover "query" [--input=results.json] [--filter-domain=example.com] [--output=file]');
        }
        $data = (new SearchDiscoveryEngine())->discover((string) $query, [
            'provider' => $this->optString($opts, 'provider', 'offline'),
            'input' => $this->optString($opts, 'input'),
            'limit' => (int) $this->opt($opts, 'limit', 10),
            'goal' => $this->optString($opts, 'goal', ''),
            'filter-domain' => $this->optString($opts, 'filter-domain') ?: $this->optString($opts, 'domain'),
        ]);
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $data);
        ...
        if ($this->bool($opts, 'json') || !$output) {
            ...
            $this->out('Search seeds output: ' . $output);
            $this->out('Seeds: ' . (string) ($data['seeds_total'] ?? 0));
        }
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function searchToSeeds(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        if (!$input || !is_file((string) $input)) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper search:to-seeds <search.json> [--filter-domain=example.com] [--format=txt|json] [--output=seeds.txt]');
        }
        ...
        $output = $this->optString($opts, 'output');
        $data = ['ok' => true, 'search_seed_export_version' => '1.0.3', 'source' => $input, 'seeds_total' => count($seeds), 'seeds' => $seeds];
        if ($output) {
            $this->ensureDir(dirname($output));
            if ($format === 'json') {
                file_put_contents($output, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
            } else {
                file_put_contents($output, implode("\n", array_map(static fn(array $row): string => (string) ($row['url'] ?? ''), $seeds)) . "\n");
            }
        }
        if ($this->bool($opts, 'json') || !$output) {
            $this->outJson($data);
        } else {
            $this->out('Seed URLs exported: ' . $output);
    ...
    private function mailToSeeds(array $args, array $opts): int
    {
        ...
        $data = [
            'ok' => true,
            'mail_seed_export_version' => '1.0.3',
            'source' => $input,
    ...
    private function extractionDictionary(array $args, array $opts): int
    {
        ...
        $learned = $dictionary->learn(strip_tags($text), (int) $this->opt($opts, 'min-length', 3), $this->optString($opts, 'source-url'), $this->optString($opts, 'category'));
        $dictionary->save($dictionaryFile);
        $data = [
            'ok' => true,
            'dictionary_version' => '1.0.3',
    ...
    private function extractionPatterns(array $args, array $opts): int
    {
        ...
        if (!$input) {
            $this->outJson(['pattern_version' => '1.0.3', 'patterns' => $registry->all()]);
            return 0;
        }
        $text = is_file($input) ? (string) file_get_contents($input) : (string) $input;
        $data = ['pattern_version' => '1.0.3', 'source' => $input, 'matches' => $registry->match($text), 'patterns' => array_keys($registry->all())];
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $data);
        }
    ...
    private function extractionMappings(array $args, array $opts): int
    {
        ...
        if (!$input) {
            $this->outJson(['mapping_version' => '1.0.3', 'mappings' => $registry->all()]);
            return 0;
        }
        ...
        $result = ['mapping_version' => '1.0.3', 'mapping' => $mapping, 'records_total' => count($records), 'records' => $mapped];
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $result);
        }
    ...
    /** @param array<string,mixed> $opts */
    private function extractionRecipes(array $opts): int
    {
        $dir = $this->optString($opts, 'dir') ?: dirname(__DIR__, 2) . '/config/extraction/recipes';
        $data = [
            'recipe_catalog_version' => '1.0.3',
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function extractionRecipe(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        $recipeFile = $this->optString($opts, 'recipe') ?: $this->optString($opts, 'file');
        if (!$input) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper extract:recipe <html-file|url> --recipe=config/extraction/recipes/generic-page.json [--output=file]');
        }
        if (!$recipeFile) {
            $recipeFile = dirname(__DIR__, 2) . '/config/extraction/recipes/generic-page.json';
        }
        [$html, $baseUrl, $source] = $this->htmlInput($input, $opts);
        $recipe = ExtractionRecipe::fromFile($recipeFile);
        $data = (new ExtractionRecipeRunner())->run($html, $baseUrl, $recipe);
        $data['source'] = $source;
        $data['recipe_file'] = $recipeFile;
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $data);
        }
        if ($this->bool($opts, 'json') || !$output) {
            ...
            $this->out('Recipe extraction output: ' . $output);
            $this->out('Quality: ' . (string) ($data['quality']['label'] ?? '') . ' score=' . (string) ($data['quality']['score'] ?? ''));
        }
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function extractionQuality(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input');
        if (!$input || !is_file($input)) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper extract:quality <extraction-result.json> [--required-field=title --required-field=doi] [--output=file]');
        }
        $data = json_decode((string) file_get_contents($input), true);
        if (!is_array($data)) {
            throw new \InvalidArgumentException('Extraction result JSON is invalid: ' . $input);
        }
        $required = [];
        foreach ((array) ($opts['required-field'] ?? []) as $field) {
            $required[] = (string) $field;
        }
        if ($required === [] && isset($data['recipe']['id']) && str_contains((string) $data['recipe']['id'], 'article')) {
            $required = ['title', 'doi'];
        }
        $report = (new ExtractionQualityReporter())->report($data, $required);
        $result = ['ok' => true, 'source' => $input, 'quality' => $report];
        $output = $this->optString($opts, 'output');
        if ($output) {
            $this->writeJson($output, $result);
        }
    ...
    private function exportConnectorList(array $args, array $opts): int
    {
        ...
        $data = [
            'export_connector_version' => '1.0.3',
    ...
    private function exportConnectorShow(array $args, array $opts): int
    {
        ...
        $this->outJson(['ok' => true, 'export_connector_version' => '1.0.3', 'connector' => $connector]);
        return 0;
    }
...
untimeException('Usage: php bin/mnb-scraper schedule:disable <schedule-id>'); }
    ...
    private function publisherList(array $args, array $opts): int
    {
        ...
        $data = [
            ...
            'publishers' => array_map(static function (array $row): array {
                return [
                    ...
                    'recommended_modes' => $row['recommended_modes'] ?? [],
                    'risk' => $row['risk'] ?? null,
                    'delay_ms' => $row['default_delay_ms'] ?? null,
                    'max_pages' => $row['default_max_pages'] ?? null,
        ...
        $format = strtolower($this->optString($opts, 'format', 'json') ?? 'json');
        if ($output) {
            $this->exportRows((array) $data['publishers'], $output, $format, $data);
            $this->out('Publisher catalog exported: ' . $output);
        }
        if ($this->bool($opts, 'json') || !$output) {
            $this->outJson($data);
        }
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherShow(array $args, array $opts): int
    {
        $id = $args[0] ?? $this->optString($opts, 'publisher') ?? $this->optString($opts, 'id');
        if (!$id) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper publisher:show <publisher-id|name> [--json]');
        }
        $catalog = new PublisherCatalog($this->rootDir);
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherSeeds(array $args, array $opts): int
    {
        $catalog = new PublisherCatalog($this->rootDir);
        $rows = $catalog->seedRows($this->optString($opts, 'risk'), $this->optString($opts, 'mode'));
        $format = strtolower($this->optString($opts, 'format', 'txt') ?? 'txt');
        $output = $this->optString($opts, 'output') ?: $this->storagePath('publisher-seeds-' . date('Ymd-His') . '.' . ($format === 'csv' ? 'csv' : ($format === 'json' ? 'json' : 'txt')));
        if ($format === 'txt') {
            $this->writeTextLines($output, array_values(array_unique(array_map(static fn (array $row): string => (string) $row['url'], $rows))));
        } else {
            $this->exportRows($rows, $output, $format, ['publisher_seed_version' => '1.0.3', 'rows_total' => count($rows)]);
        }
        $data = ['ok' => true, 'publisher_seed_version' => '1.0.3', 'seeds_total' => count($rows), 'format' => $format, 'output' => $output];
        $this->bool($opts, 'json') ? $this->outJson($data) : $this->out('Publisher seeds exported: ' . $output);
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherPlan(array $args, array $opts): int
    {
        $catalog = new PublisherCatalog($this->rootDir);
        $jobs = $catalog->planJobs(
            (int) $this->opt($opts, 'max-pages', 10),
            (int) $this->opt($opts, 'delay-ms', $this->opt($opts, 'gap-ms', 3500)),
            $this->optString($opts, 'risk'),
            $this->optString($opts, 'mode')
        );
        $data = [
            'ok' => true,
            'publisher_plan_version' => '1.0.3',
            'jobs_total' => count($jobs),
            'jobs' => $jobs,
            'policy' => (string) ($catalog->meta()['policy'] ?? ''),
        ];
        $output = $this->optString($opts, 'output') ?: $this->storagePath('publisher-crawl-jobs-' . date('Ymd-His') . '.json');
        $this->writeJson($output, $data);
        if ($this->bool($opts, 'json')) {
            $this->outJson($data + ['output' => $output]);
        } else {
            $this->out('Publisher crawl plan: ' . $output);
            $this->out('Jobs: ' . count($jobs));
        }
        return 0;
    }
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherGraph(array $args, array $opts): int
    {
        $id = $args[0] ?? $this->optString($opts, 'publisher') ?? $this->optString($opts, 'id');
        if (!$id) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper publisher:graph <publisher-id|name> [--json]');
        }
        $publisher = (new PublisherCatalog($this->rootDir))->find((string) $id);
        $graph = new PublisherContentGraph($publisher);
        $data = [
            'ok' => true,
            'publisher_graph_version' => '1.0.3',
        ...
        if ($output) {
            $this->writeJson($output, $data);
            $this->out('Publisher graph exported: ' . $output);
        }
        if ($this->bool($opts, 'json') || !$output) {
            $this->outJson($data);
        }
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherEnterprisePlan(array $args, array $opts): int
    {
        $id = $args[0] ?? $this->optString($opts, 'publisher') ?? $this->optString($opts, 'id');
        if (!$id) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper publisher:enterprise-plan <publisher-id|name> [--max-journals=10] [--max-books=10] [--max-issues=10] [--max-articles=25] [--output=file]');
        }
        $publisher = (new PublisherCatalog($this->rootDir))->find((string) $id);
        $plan = (new PublisherContentGraph($publisher))->enterprisePlan(
            (int) $this->opt($opts, 'max-journals', 10),
            (int) $this->opt($opts, 'max-books', 10),
            (int) $this->opt($opts, 'max-issues', 10),
            (int) $this->opt($opts, 'max-articles', 25)
        );
        $data = ['ok' => true, 'publisher_enterprise_plan_version' => '1.0.3', 'plans_total' => count($plan), 'plans' => $plan];
        $output = $this->optString($opts, 'output') ?: $this->storagePath('publisher-enterprise-plan-' . date('Ymd-His') . '.json');
        $this->writeJson($output, $data);
        if ($this->bool($opts, 'json')) {
            $this->outJson($data + ['output' => $output]);
        } else {
            $this->out('Publisher enterprise plan: ' . $output);
            $this->out('Plans: ' . count($plan));
        }
        return 0;
    }

    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherExtractArticle(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input') ?? $this->optString($opts, 'file');
        if (!$input || !is_file($input)) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper publisher:extract-article <saved-article.html> [--publisher=springer] [--url=https://...] [--output=file]');
        }
        $publisherId = $this->optString($opts, 'publisher') ?: 'springer';
        $publisher = (new PublisherCatalog($this->rootDir))->find((string) $publisherId);
        $record = (new PublisherArticleExtractor($publisher))->extractFromHtml((string) file_get_contents($input), $this->optString($opts, 'url'));
        $data = [
            'ok' => true,
            'publisher_article_extract_version' => '1.0.3',
        ...
        if ($output) {
            $this->writeJson($output, $data);
            $this->out('Article metadata extracted: ' . $output);
        }
        if ($this->bool($opts, 'json') || !$output) {
            $this->outJson($data);
        }
    ...
    private function publisherSchema(array $args, array $opts): int
    {
        ...
        $data = ['ok' => true, 'article_schema_version' => '1.0.3', 'fields_total' => count($rows), 'fields' => $rows];
        $output = $this->optString($opts, 'output');
        $format = strtolower($this->optString($opts, 'format', 'json') ?? 'json');
        if ($output) {
            $this->exportRows($rows, $output, $format, $data);
            $this->out('Article schema exported: ' . $output);
        }
        if ($this->bool($opts, 'json') || !$output) {
            $this->outJson($data);
        }
    ...
    /** @param array<int,string> $args @param array<string,mixed> $opts */
    private function publisherNormalize(array $args, array $opts): int
    {
        $input = $args[0] ?? $this->optString($opts, 'input') ?? $this->optString($opts, 'file');
        if (!$input || !is_file($input)) {
            throw new \InvalidArgumentException('Usage: php bin/mnb-scraper publisher:normalize <records.json> [--publisher=id] [--format=json|csv|jsonl] [--output=file]');
        }
        $data = json_decode((string) file_get_contents($input), true);
        if (!is_array($data)) {
            throw new \RuntimeException('Invalid JSON records file: ' . $input);
        }
        $records = $this->recordsFromData($data);
        $publisherId = $this->optString($opts, 'publisher') ?: $this->optString($opts, 'id');
        $publisherName = null;
        if ($publisherId) {
            try {
                $publisherName = (string) ((new PublisherCatalog($this->rootDir))->find((string) $publisherId)['publisher'] ?? $publisherId);
            } catch (\Throwable) {
                $publisherName = (string) $publisherId;
            }
        }
        $rows = (new ArticleMetadataNormalizer())->normalizeMany($records, $publisherName);
        $format = strtolower($this->optString($opts, 'format', 'json') ?? 'json');
        ...
        $this->exportRows($rows, $output, $format, [
            'article_metadata_version' => '1.0.3',
            'source' => $input,
            'records_total' => count($records),
            'normalized_total' => count($rows),
            'publisher' => $publisherName,
        ]);
        $result = ['ok' => true, 'article_metadata_version' => '1.0.3', 'records_total' => count($records), 'normalized_total' => count($rows), 'output' => $output];
