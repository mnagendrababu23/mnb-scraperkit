...
trait HardeningCommandTrait
{
    ...
    {
        $report = (new ProductionReadinessInspector())->inspect($this->rootDir);
        if ($this->bool($opts, 'json')) {
            $this->outJson($report);
        } else {
            $this->out('MNB ScraperKit ' . ($report['hardening_version'] ?? '4.0.1') . ' production readiness');
            $this->out('Status: ' . strtoupper((string) ($report['status'] ?? 'unknown')) . ' | Score: ' . (string) ($report['score'] ?? 0) . '/100');
            $this->out('');
